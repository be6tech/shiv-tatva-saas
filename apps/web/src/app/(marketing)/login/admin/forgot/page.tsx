...
export default function AdminForgotPasswordPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error === "not_configured" ? "Password reset is not configured on the server." : "Request failed.");
        return;
      }
    ...
    <div className={cn(marketingPageRoot, "relative grid min-h-screen place-items-center px-4 py-12")}>
      ...
      >
        ...
        <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">
          Enter your admin email. A one-time reset link is stored in the database (valid 1 hour).
        </p>
